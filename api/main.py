...
# guard keeps dev mode (no build present) booting cleanly. On Vercel, the
# frontend build also ends up bundled alongside this function (both come from
# the same build checkout), so this same mount is what serves "/" and the JS/CSS
# assets there too — there is no separate static deployment to rely on instead.
if _DIST_DIR.is_dir():
